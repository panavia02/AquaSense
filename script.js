...
class Guests {
	...
	addGuests(guestsData) {
		...
		this.updateTimer = setInterval(getUpdatedData, 5000);
	}
	...
	addGuest(guestData) {
		this.data[guestData.guest_id] = new Guest(guestData.guest_id, guestData.guest_section, guestData.guest_room, guestData.guest_name, guestData.guest_time);
...
class Guest {
	constructor(id, section, room, name, time) {
		this.id = id;
	...
	refreshInfo() {
		...
		this.obj.querySelector('#dashboard-location').innerHTML = `Section ${this.section} - Room ${this.room}`;
	...
	loadProfile() {
		...
		profileLocation.innerText = "Section " + this.section + " - Room " + this.room;
	...
	loadCareplan() {
		...
		careplanLocation.innerText = "Section " + this.section + " - Room " + this.room;
		//careplan_bottomElement.style.cssText = "";
	}
	
	loadAnalytics() {
		analyticsName.innerText = this.name;
		analyticsLocation.innerText = "Section " + this.section + " - Room " + this.room;
...
//get updated user data from database (polled every 5s -- see Guests.addGuests()).
async function getUpdatedData() {
    try {
		const response = await fetch('getdata.php?mode=getupdateddata&last_seen=' + encodeURIComponent(guests.lastUpdated));
        //const response = await fetch("getupdateddata.php?last_seen=" + encodeURIComponent(guests.lastUpdated));
        const dataArray = await response.json();
...
function createGuestElement(guest) {
	...
	guest.obj.querySelector('#dashboard-location').innerHTML = `Section ${guest.section} - Room ${guest.room}`;
