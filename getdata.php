...
function getData() {
    ...
    try {
        ...
        $stmt = $pdo->prepare('SELECT * FROM guests WHERE 1=1' . $clause);
        $stmt->execute($params);
        $data = $stmt->fetchAll();
...
function getUpdatedData() {
    ...
    try {
        ...
        [$clause, $params] = accessFilterClause();
        $stmt = $pdo->prepare("SELECT * FROM guests WHERE guest_updated_at > ?" . $clause . " ORDER BY guest_updated_at ASC");
...
function getGuestFacility() {
    ...
    try {
        $pdo = getPDO();
        $stmt = $pdo->prepare(
            "SELECT g.guest_id, g.guest_section AS section_id, sn.section_name, sn.location_id, l.location_name,
        ...
        $canReassign = $canChangeLocation || isLocationAdmin($pdo, $currentUser['user_id'], $locationId);
...
function reassignGuest() {
    ...
    try {
        ...
        $stmt = $pdo->prepare(
            "UPDATE guests SET guest_section = :sec, guest_room = :room, guest_updated_at = NOW() WHERE guest_id = :id"
        );
